...
export const state = {
    user: {
        ...
        stats: {
            ...
            xp: 0,
            nextLevelXp: 1000,
            gamesCount: { mines: 0, crash: 0, blackjack: 0, dice: 0 }
        },
        quests: [
            { id: 'q1', title: 'Исследователь Недр', desc: 'Сыграть 15 игр в Mines', target: 15, current: 0, reward: 0.3, rewardLabel: '+30%', minDep: 1500, unlocked: false },
            { id: 'q2', title: 'Разрушитель Графиков', desc: 'Сыграть 30 игр в Crash', target: 30, current: 0, reward: 0.4, rewardLabel: '+40%', minDep: 3000, unlocked: false },
            { id: 'q3', title: 'Легенда Азарта', desc: 'Достичь 10 уровня', target: 10, current: 1, reward: 0.5, rewardLabel: '+50%', minDep: 5000, unlocked: false },
            { id: 'q4', title: 'Властелин Nexus', desc: 'Сделать оборот 50 000 ₽', target: 50000, current: 0, reward: 0.7, rewardLabel: '+70%', minDep: 10000, unlocked: false }
        ]
    },
    rooms: [],
    listeners: [],
    ...
    // Имитация депозита с учетом твоей финансовой модели
    applyDeposit(amount) {
        const fee = amount * 0.10; // Твои 10% профита на входе
        const netAmount = amount - fee;
        
        // Поиск максимально доступного бонуса по квестам
        let bonusPercent = 0;
        const availableQuests = this.user.quests.filter(q => q.unlocked && amount >= q.minDep);
        if (availableQuests.length > 0) {
            bonusPercent = Math.max(...availableQuests.map(q => q.reward));
        }

        const bonusValue = netAmount * bonusPercent;
        
        this.user.balance += netAmount;
        if (bonusValue > 0) {
            this.user.wager.bonusLocked += bonusValue;
            this.user.wager.target += bonusValue * 10; // Вейджер х10
        }

        this.save();
        this.notify();
        return { netAmount, bonusValue, fee };
    },

    updateBalance(amount, isWin = false) {
        if (!isWin && amount < 0) {
            const betSize = Math.abs(amount);
            this.user.wager.current += betSize;
            
            // Проверка выполнения вейджера
    ...
    save() { if (this.user.id !== 0) localStorage.setItem(`nexus_data_${this.user.id}`, JSON.stringify(this.user)); },
    load(tgUser) {
        if (!tgUser) return;
